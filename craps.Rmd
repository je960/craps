...
#number of rows
n <- number_of_contests * contest_size

x <- tibble(ishan = map_int(1:n, ~ roll_two_dice()), 
            john = map_int(1:n, ~ roll_two_dice())) %>%
     mutate(john_win = john %in% c(7,11)) %>% 
     mutate(ishan_win = ishan %in% c(7, 11)) %>%  
  #create a column with just says "Tie" because it will default to tie 
     mutate(winner = "Tie") %>% 
  #because they are boolean, you can just use ! or not
     mutate(winner = ifelse(ishan_win & !john_win, "Ishan", winner), 
            winner = ifelse(!ishan_win & john_win, "John", winner)) %>% 
     mutate(contest = rep(1:number_of_contests, each = contest_size) %>% 
     group_by(contest) %>% 
     summarize(john_win = sum(winner =="JM"))
x
